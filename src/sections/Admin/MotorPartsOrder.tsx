...
const MotorPartsOrder = () => {
    ...
    const handleStatusToggle = async (order: MotorPartOrder, e: React.MouseEvent) => {
        e.stopPropagation();
    ...
    const groupedOrdersByCategory = useMemo(() => {
        const groups: { [key: string]: MotorPartOrder[] } = {
            'Today': [],
            'Yesterday': [],
            'Older': []
        };
        ...
        <div className="space-y-8 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-16" onClick={() => setActiveDropdown(null)}>
            <div className="flex items-center gap-4">
                ...
                <Header
                    ...
                    description="Track walk-in component purchases, categories, pricing, and payment timelines."
                />
            </div>
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4 bg-white/60 backdrop-blur-xl p-3 border border-slate-200/60 rounded-2xl shadow-2xs">
                ...
                <div className="relative flex-1 max-w-sm">
                    ...
                    <input
                        type="text"
                        placeholder="Search customer, brand, part..."
                        value={searchQuery}
            ...
            {loading ? (
                ...
                <div className="bg-white/60 backdrop-blur-xl border border-slate-200/60 rounded-3xl p-16 text-center space-y-4 shadow-2xs">
                    ...
                    <div className="space-y-1 max-w-sm mx-auto">
                        <h3 className="text-base font-semibold text-slate-900">No motor part orders found</h3>
                        <p className="text-slate-500 text-xs leading-relaxed">
                            {searchQuery ? 'Try adjusting your search criteria or filters.' : 'Walk-in customer component transactions and purchase records will appear here once logged.'}
                        </p>
                    </div>
                </div>
            ) : (
                                ...
                                <table className="w-full text-left border-collapse min-w-[750px]">
                                    <thead>
                                        <tr className="border-b border-slate-100 text-[11px] font-semibold text-slate-400 uppercase tracking-wider">
                                            ...
                                            <th className="py-3 px-6">Part / Brand</th>
                                        ...
                                        {dateOrders.map((order) => {
                                            ...
                                            return (
                                                <tr key={order.$id} className="hover:bg-slate-50/60 transition-colors group">
                                                    ...
                                                    <td className="py-4 px-6 font-mono text-slate-600">{order.category}</td>
