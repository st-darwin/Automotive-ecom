...
const TyreOrderComponent = () => {
    ...
    // Slider scroll ref for metrics cards on mobile
    ...
    // Tyre Revenue & Metrics calculations
    ...
    // Filtered orders based on active tab and search query (sorted newest first)
    ...
    // Group filtered orders into "Today", "Yesterday", and "Older"
    ...
    return (
        <div className="space-y-8 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-16" onClick={() => setActiveDropdown(null)}>
            ...
            <div className="flex items-center gap-4">
                ...
                <Header
                    title="Tyre Orders Management"
                    description="Track walk-in customer purchases, tyre quantities, pricing, and payment timelines."
                />
            </div>

            {/* Metrics Overview Carousel / Slider */}
            <div className="relative group">
                <div className="absolute -left-3 top-1/2 -translate-y-1/2 z-10 hidden sm:flex items-center justify-center">
                    <button 
                        onClick={() => scrollSlider('left')}
                        className="p-2 rounded-full bg-white shadow-md border border-slate-200 text-slate-600 hover:bg-slate-50 cursor-pointer"
                    >
                ...
                <div 
                    ref={sliderRef}
                    className="flex sm:grid sm:grid-cols-2 lg:grid-cols-4 gap-5 overflow-x-auto sm:overflow-x-visible snap-x snap-mandatory scrollbar-none pb-2 pt-1 px-1"
                >
                    ...
                    <div className="min-w-[260px] sm:min-w-0 snap-start bg-white/80 backdrop-blur-xl border border-slate-200/60 rounded-3xl p-5 shadow-2xs flex items-center gap-4 flex-shrink-0">
                        ...
                        <div>
                            <span className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider block mb-0.5">Month Tyre Revenue</span>
                            <h3 className="text-xl font-bold text-slate-900">₦{loading ? '...' : metrics.currentMonthTyreRevenue.toLocaleString()}</h3>
