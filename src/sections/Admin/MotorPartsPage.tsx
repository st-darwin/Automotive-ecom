...
import { Plus, MoreVertical, Edit3, Trash2, Wrench, Search } from 'lucide-react';
...
const MotorPartsPage = () => {
    const navigate = useNavigate();
    const [parts, setParts] = useState<MotorPart[]>([]);
    ...
    const fetchParts = async () => {
        try {
            ...
            const response = await database.listDocuments(
                ...
                appwriteConfig.motorPartsCollection, // Make sure this is defined in your Appwrite config
            );
            setParts(response.documents as unknown as MotorPart[]);
        } catch (error) {
            console.error('Error fetching motor parts inventory:', error);
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleDelete = async (id: string, e: React.MouseEvent) => {
        e.stopPropagation();
        if (!window.confirm('Are you sure you want to delete this motor part?')) return;
    ...
    const handleEdit = (part: MotorPart, e: React.MouseEvent) => {
        e.stopPropagation();
        navigate('/inventory/motor-parts/create', { state: { partToEdit: part } });
    };
    ...
    const filteredParts = parts.filter((part) => {
        ...
        return (
            ...
            String(part.year)?.toLowerCase().includes(query)
        );
    });

    const renderPartMedia = (part: MotorPart, isMobile: boolean = false) => {
        ...
        <div className="space-y-6 sm:space-y-8 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-16" onClick={() => setActiveDropdown(null)}>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <Header
                    ...
                    description="Manage motor parts, components, categories, stock, and pricing."
                />
                <button
                    onClick={() => navigate('/inventory/motor-parts/create')}
            ...
            <div className="relative max-w-md w-full">
                ...
                <input
                    type="text"
                    placeholder="Search by name, brand, category, or year..."
                    value={searchQuery}
                ...
                <div className="bg-white/60 backdrop-blur-xl border border-slate-200/60 rounded-3xl p-8 sm:p-16 text-center space-y-4 shadow-2xs">
                    ...
                    <div className="space-y-1 max-w-sm mx-auto">
                        <h3 className="text-base font-semibold text-slate-900">
                            {parts.length === 0 ? "No motor parts in inventory" : "No matching motor parts found"}
                        </h3>
                        <p className="text-slate-500 text-xs leading-relaxed">
                            {parts.length === 0 
                                ? "Get started by creating your first motor part with category, brand, pricing, and year details."
                            ...
                            >
                                <div className="flex items-start justify-between gap-3">
                                    <div className="flex items-center gap-3">
                                        {renderPartMedia(part, true)}
                                        <div>
                                            <h4 className="font-semibold text-slate-900 text-sm group-hover:text-blue-600 transition-colors line-clamp-1">
                                                {part.name}
                                            </h4>
                                            <p className="text-xs text-slate-500">{part.brand} &bull; <span className="font-mono">{part.category}</span></p>
                                        </div>
                                    </div>
                                    ...
                                    <div className="relative" onClick={e => e.stopPropagation()}>
                                        <button
                                            onClick={(e) => {
                                                e.stopPropagation();
                                                setActiveDropdown(activeDropdown === part.$id ? null : part.$id);
                                            }}
                                ...
                                <div className="grid grid-cols-4 gap-2 pt-3 border-t border-slate-100 text-[11px]">
                                    <div>
                                        ...
                                        <span className="font-semibold text-slate-900 block">{part.year || 'N/A'}</span>
                                    </div>
                                    <div>
                                        <span className="text-slate-400 block mb-0.5">Category</span>
                                        <span className="font-semibold text-blue-600 truncate block">{part.category}</span>
                                    </div>
                                    <div>
                                        <span className="text-slate-400 block mb-0.5">Price</span>
                                        <span className="font-semibold text-slate-900 block">₦{part.price?.toLocaleString()}</span>
                                    </div>
                                    <div>
                                        <span className="text-slate-400 block mb-0.5">Stock</span>
                                        <span className={`font-semibold ${part.stock > 5 ? 'text-emerald-600' : 'text-amber-600'}`}>
                                            {part.stock} units
                                        </span>
                                    </div>
                                </div>
                            ...
                            <table className="w-full text-left border-collapse">
                                <thead>
                                    <tr className="border-b border-slate-100 bg-slate-50/50 text-[11px] font-semibold text-slate-400 uppercase tracking-wider">
                                        <th className="py-4 px-6">Part Item</th>
                                    ...
                                    {filteredParts.map((part) => (
                                        ...
                                        >
                                            <td className="py-4 px-6">
                                                <div className="flex items-center gap-3">
                                                    {renderPartMedia(part, false)}
                                                    <div>
                                                        <p className="font-semibold text-slate-900 group-hover:text-blue-600 transition-colors">
                                                            {part.name}
                                                        </p>
                                                        <span className="text-[10px] text-slate-400 font-mono">ID: {part.$id.slice(0, 8)}...</span>
                                                    </div>
                                                </div>
                                            </td>
                                            <td className="py-4 px-6">
                                                <div className="space-y-0.5">
                                                    <p className="text-slate-900 font-semibold">{part.brand}</p>
                                                    <span className="text-[11px] text-slate-400">{part.category}</span>
                                                </div>
                                            </td>
                                            ...
                                            <td className="py-4 px-6">
                                                <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-semibold ${
                                                    part.stock > 5 ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'
                                                }`}>
                                                    <span className={`w-1.5 h-1.5 rounded-full ${part.stock > 5 ? 'bg-emerald-500' : 'bg-amber-500'}`} />
                                                    {part.stock} units
                                                </span>
                                            </td>
