...
const CreateGrease = () => {
    ...
    const greaseToEdit = location.state?.greaseToEdit || null;
    ...
    useEffect(() => {
        if (greaseToEdit) {
            setFormData({
                name: greaseToEdit.name || '',
                brand: greaseToEdit.brand || 'Boothman',
                volume: greaseToEdit.volume || '',
                price: greaseToEdit.price ? String(greaseToEdit.price) : '',
                stock: greaseToEdit.stock ? String(greaseToEdit.stock) : '',
                imageUrl: greaseToEdit.imageUrl || '',
                year: greaseToEdit.year ? String(greaseToEdit.year) : new Date().getFullYear().toString(),
            });
            if (greaseToEdit.imageUrl) {
                setImagePreview(greaseToEdit.imageUrl);
            }
        }
    }, [greaseToEdit]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            navigate('/inventory/grease');
        } catch (error) {
            console.error('Error saving grease item:', error);
        } finally {
            setSubmitting(false);
        }
    };
    ...
    return (
        <div className="space-y-8 max-w-3xl mx-auto pb-16">
            <div className="flex items-center gap-4">
                <button
                    onClick={() => navigate('/inventory/grease')}
                ...
                <Header
                    title={greaseToEdit ? 'Edit Grease Item' : 'Create Grease Item'}
                    description={greaseToEdit ? 'Update grease specifications and stock metrics.' : 'Add a brand new grease variant to the inventory catalog.'}
                />
            </div>
            ...
            <form onSubmit={handleSubmit} className="bg-white/80 backdrop-blur-xl border border-slate-200/60 rounded-3xl p-8 space-y-6 shadow-2xs">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                        ...
                        <input
                            type="text"
                            required
                            placeholder="e.g. Boothman EP2 High-Temp Grease"
                            value={formData.name}
                    ...
                    <div className="space-y-2">
                        <label className="text-xs font-semibold text-slate-700 tracking-wide uppercase">Volume / Size</label>
                        <input
                            type="text"
                            required
                            placeholder="e.g. 18kg, 500g, Cartridge"
                    ...
                    <div className="space-y-2">
                        <label className="text-xs font-semibold text-slate-700 tracking-wide uppercase">Production Year</label>
                        <input
                            type="number"
                            required
                            placeholder="2026"
                ...
                <div className="space-y-2">
                    ...
                    <div
                        ...
                        className={`relative border-2 border-dashed rounded-3xl p-6 text-center transition-all flex flex-col items-center justify-center ${
                            isDragging ? 'border-amber-500 bg-amber-50/40' : 'border-slate-200 bg-slate-50/30 hover:bg-slate-50'
                        }`}
                    >
                        {imagePreview ? (
                            ...
                            <div className="space-y-3 py-4">
                                ...
                                <div className="space-y-1">
                                    <p className="text-xs font-semibold text-slate-700">
                                        Drag & drop grease image here, or <label className="text-amber-600 cursor-pointer hover:underline">browse</label>
                                    </p>
                ...
                <div className="pt-4 flex items-center justify-end gap-3 border-t border-slate-100">
                    ...
                    <button
                        type="submit"
                        disabled={submitting}
                        className="inline-flex items-center gap-2 px-6 py-3 rounded-2xl bg-amber-600 hover:bg-amber-700 text-white text-xs font-semibold transition-all shadow-sm shadow-amber-500/20 active:scale-95 disabled:opacity-50 cursor-pointer"
                    >
                        {submitting && <Loader2 className="w-4 h-4 animate-spin" />}
                        <span>{greaseToEdit ? 'Update Grease' : 'Save Grease'}</span>
                    </button>
                </div>
            </form>
        </div>
